...
class Bandit:
    ...
    def __init__(self, num_arms, true_rewards, epsilon, mode):
    ...
    def __randomly_increment_rewards(self):
        self.true_rewards = (
            [self.__randomly_increment(reward) for reward in self.true_rewards]
        )

    @staticmethod
    def __randomly_increment(value):
...
if __name__ == "__main__":
    ...
    __NUM_ARMS = 5
    __NUM_PULLS = 10000
    __SIMULATION_STATUS_INTERVAL = 100
    __EPSILON = 0.1
    __RUN1 = simulate(
        __NUM_SIMULATIONS,
        __NUM_ARMS,
    ...
    plt.legend([
        f"mode={Mode.SAMPLE_AVERAGE.value}",
        f"mode={Mode.CONSTANT.value}",
    ])
